...
pub fn lookup_named_number(named_number: &NamedNumber) -> Rational {
	match named_number {
		...
		Duodecillion      => Rational::from_sci_string("1E+39").unwrap(),
		Tredecillion      => Rational::from_sci_string("1E+42").unwrap(),
		Quattuordecillion => Rational::from_sci_string("1E+45").unwrap(),
		Quindecillion     => Rational::from_sci_string("1E+48").unwrap(),
		Sexdecillion      => Rational::from_sci_string("1E+51").unwrap(),
		Septendecillion   => Rational::from_sci_string("1E+54").unwrap(),
		Octodecillion     => Rational::from_sci_string("1E+57").unwrap(),
		Novemdecillion    => Rational::from_sci_string("1E+60").unwrap(),
		Vigintillion      => Rational::from_sci_string("1E+63").unwrap(),
		Googol            => Rational::from_sci_string("1E+100").unwrap(),
		Centillion        => Rational::from_sci_string("1E+303").unwrap(),
	}
}
